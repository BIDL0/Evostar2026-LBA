...
#   Usage:
#
#   ./BiCAS.py [integer_ca_size] [ltf_file.tab] [istate_file.cas]
#
#   ! The arguments are optional but their order is important !
#
# ******************************************************************************
# This program requires the Python3 environment with NumPy and PyGame libraries.
#
# ------------------------------------ Licence ---------------------------------
...
#                 E-mail: see http://www.fit.vut.cz/person/bidlo
#
# ******************************************************************************
...
_WIN = 800
# if you wish to slow-down the CA development, specify
# the WAIT_TIME > 0 (in milliseconds, e.g. 200)
WAIT_TIME = 0

# color constants for cell states => maximum 13 states
...
color = [ black, yellow, white, blue, red, green, brown, orange, cyan, violet,
          gray, magenta, pink ]
...
class CA:
    def __init__(self, rows=20, cols=20, states=2, nsize=5):
        ...
        self.hx = 220 # help text coordinate offsets
        self.hy = 0
        self.hy0 = 470
    ...
    def set_clicked_cell(self, xy):
        pos_x, pos_y = xy
        row = int(pos_y / self.cell_h + 1)
        col = int(pos_x / self.cell_w + 1)
        ...
        for row in range(1, self.rows+1):
            for col in range(1, self.cols+1):
                pygame.draw.rect(win, color[self.get_cell(row, col)],
                                (int((col-1)*self.cell_w), int((row-1)*self.cell_h),
                                int(self.cell_w), int(self.cell_h)), 0)
                pygame.draw.rect(win, blue,
                                (int((col-1)*self.cell_w), int((row-1)*self.cell_h),
                                int(self.cell_w), int(self.cell_h)), 1)
        pygame.display.update()
        # show help after startapp
        if self.shelp == 1:
            self.show_help(win)
    ...
    def read_state(self, sfile):
        ...
        for r in range(0, len(istate)):
            for s in range(0, len(istate[0])):
                self.istt[int(r+roff+1)][int(s+coff+1)] = istate[int(r)][int(s)]
        self.istt_init()
    ...
    def read_ca(self, file_name):
        ...
        with caf:
            for line in caf:
                ...
                if row == 0: # headline: #states [istate_filename]
                    if len(line) < 1 and len(line) > 3:
                        # if the headline does not meet the requirements...
                        print("The head of tab file must be: number_of_states [initial_stste_file]")
                        sys.exit()
                    # parsing the headline
                    if len(line) == 3: # just for compatibility reasons...
                        # NOTE: line[0] could be the CA dimensions... ignoring, here it is ALWAYS 2
                        if line[1].isdigit(): # the 2nd element must bee a number... of the CA states
                            self.states = int(line[1])
                        self.state_file_custom(line[2]) # the 3rd element assumes the name of .cas file...
                        self.istt_init() # ...from which the CA is initialised
                    if len(line) == 2:
                        if line[1].isdigit(): # if the 2nd element is digit --> the num. of CA states
                            self.states = int(line[1])
                        else: # otherwise it will be interpreted as the istate_filename
                            self.state_file_custom(line[1]) # read the initial state...
                            self.istt_init() # ...and initialise the CA
                            if line[0].isdigit(): # bu the 1st element must bee a number... of the CA states
                                self.states = int(line[0])
                            else: # if it is not --> error
                                print("The head of tab file must be: number_of_states [initial_stste_file]")
                                sys.exit()
                    if len(line) == 1: # at least the numner of stated must be given in the headline
                        if line[0].isdigit():
                            self.states = int(line[0])
                        else: # otherwise the headline is incorrect --> exit
                            print("The head of tab file must be: number_of_states [initial_stste_file]")
                            sys.exit()
                elif line[0][0] != "#": # reading the transition rules (rows)
    ...
    while True: # interactive keyboard control
        ...
        if devel == 1:
            ...
            if WAIT_TIME > 0:
                ...
                # specify the WAIT_TIME parameter above.
